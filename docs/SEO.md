...
- **Project page structure** - Every project route follows the same two-file pattern:
  - `app/<slug>/page.tsx` - server component shell. Exports `metadata` and `structuredData`, renders `<JsonLd>` and the content component. Around ~15 lines.
  - `app/<slug>/<slug>-content.tsx` - the actual page body (images, copy, links). **Stays a server component by default**. Add `"use client"` only when the file actually needs client-only React features (`useState`, `useEffect`, event handlers on built-in elements). Example: `app/bloom-ai/bloom-ai-content.tsx` uses `"use client"` because its video modal needs `useState`. The other four content files don't.
...
2. **Create the route as a server shell** - `app/new-project/page.tsx` exports `metadata` + `structuredData` and renders `<JsonLd>` + `<NewProjectContent />`. Follow the scaffold shown in the "Project page structure" section above.
3. **Create the content component** - `app/new-project/new-project-content.tsx` with the page body, default export named `NewProjectContent`. Add `"use client"` only if the file needs state or browser-only APIs.
...
- **Client components can't export `metadata`** - the metadata export must live on the server shell (`page.tsx`). Don't move `metadata` into a `"use client"` content file.
- **Don't add `"use client"` to content files unless they need it** - server components render once on the server, ship no JS for their body, and stream faster. Only add `"use client"` when the file uses `useState`, `useEffect`, refs, or DOM event handlers.
